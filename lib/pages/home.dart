...
import 'package:sharesdk/sharesdk_defines.dart';
...
class _HomePageState extends State<HomePage> {
  void shareSdkVersion(BuildContext context) {
    ShareSDK.sdkVersion.then((dynamic version) {
      if (version.length > 0) {
        if (Platform.isIOS) {
  ...
  void shareToDouyin(BuildContext context) {
    ...
    ShareSDK.share(ShareSDKPlatforms.douyin, params,
  ...
  void getUserInfoToWechat(BuildContext context) {
    ShareSDK.getUserInfo(ShareSDKPlatforms.wechatSession, (SSDKResponseState state, Map user, SSDKError error) {
      showAlert(state, user != null ? user : error.rawData, context);
    });
  }

  void cancelAuth(BuildContext context) {
    ShareSDK.cancelAuth(ShareSDKPlatforms.wechatSession, (SSDKResponseState state, Map user, SSDKError error) {
      showAlert(state, error.rawData, context);
    });
  }

  void hasAuthed(BuildContext context) {
    ShareSDK.hasAuthed(ShareSDKPlatforms.wechatSession, (SSDKResponseState state, Map user, SSDKError error) {
      showAlert(state, error.rawData, context);
    });
  }
  ...
  void shareSinaLinkCard(BuildContext context) {
    SSDKMap params = SSDKMap()
      ..setSinaLinkCard("linkcard_text", "linkcard_title", "http://www.mob.com/", "全新品牌，优质服务",
          "http://wx4.sinaimg.cn/large/006WfoFPly1fq0jo9svnaj30dw0dwdhv.jpg", "120", "120");
    ShareSDK.share(ShareSDKPlatforms.sina, params,
        (SSDKResponseState state, Map userdata, Map contentEntity, SSDKError error) {
      showAlert(state, error.rawData, context);
    });
